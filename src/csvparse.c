...
static void 
csvparse_error(struct csvparse *ctx,const char *fmt,...)
{
  va_list va;
...
static void 
csvparse_init(struct csvparse *ctx,char *str,char *err,int len)
{
  ctx->parse_ptr=str;
  ctx->need_nextline=0;
  ctx->errmsg=err;
...
struct csvparse* 
csvparse_new(char *str,char *err,int len)
{
  struct csvparse* ctx=(struct csvparse*)malloc(sizeof(*ctx));
  csvparse_init(ctx,str,err,len);
  return ctx;
}

void 
csvparse_delete(struct csvparse* ctx)
{
  free(ctx);
}

int 
csvparse_next_column(struct csvparse *ctx,char **out)
{
...
loop:
  ...
  goto loop;
csvparse_error:
  csvparse_error(ctx,"unexpected char %c on %s\n", *ptr, ptr);
  *out = NULL;
  return -1;
}

int 
csvparse_next_line(struct csvparse *ctx)
{
  ctx->need_nextline = 0;
...
static inline void 
csvdata_init(struct csvdata* csv)
{
  csv->col_cnt=0;
  csv->line_cnt=0;
  csv->line_cap=0;
...
const char* 
csvdata_error(struct csvdata *csv)
{
  return csv->errmsg;
}
...
void 
csvdata_delete(struct csvdata *csv)
{
  free(csv->data);
  free(csv);
}
...
int 
csvdata_parse(struct csvdata*csv,char* str)
{
  ...
  p=&parse;
  csvparse_init(p,str,csv->errmsg,sizeof(csv->errmsg));
...
const char* 
csvdata_value_string(struct csvdata *csv,int line, int row, const char* def)
{
...
long 
csvdata_value_long(struct csvdata *csv,int line, int row, long def)
{
...
double 
csvdata_value_double(struct csvdata *csv,int line, int row, double def)
{
  const char* val = csvdata_value_string(csv,line, row, (const char*)NULL);
  if (!val) return def;
...
void 
csvfile_delete(struct csvfile* csv)
{
  free(csv->data.data);
  free(csv->buf);
  free(csv);
...
int 
csvfile_load(struct csvfile* csv,const char* path)
{
  size_t len, r;
  char *b;
...
struct csvdata* 
csvfile_data(struct csvfile* csv)
{
  return &csv->data;
}
